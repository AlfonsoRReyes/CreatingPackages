...
#' @param hashSearch logical value
...
readTweetData <- function(file, hashSearch, widths=c(18, 14, 18, 1000)){
  ...
  tweetData <- as.data.frame(sapply(tweetData, str_trim), stringsAsFactors=FALSE)
...
tweetCounts <- function(tweetDF){
  ...
  tweetC <- data.frame(screenName = names(rtSplit),
                       total = nTweet,
                       original = nTweet - nRT, retweet = nRT,
